...
public class Cake extends CoffeeShop implements ManageItemInEveryCategory {
    public static void loadItemAsCake() throws IOException {
        FileInputStream inputStream = new FileInputStream("cake.txt");
    ...
    public void addItemByAdmin(String itemName, int price) {
        ...
        if (newItem != null)
            ...
            try (PrintWriter output = new PrintWriter(new FileWriter("cake.txt", true))) {
                output.printf("%s\r\n", itemName + " " + price + " " + "AVAILABLE");
                listOfCake.add(newItem);
    ...
    public void changeStateOfItemByAdmin(String itemName, ItemState state) throws IOException {
        ...
        if (item == null)
            ...
            ManageItemInEveryCategory.deleteLine("cake.txt", deletedLine);
