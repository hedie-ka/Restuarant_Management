...
public class Frise extends FastFood { ///SUB CATEGORY

    public static void loadItemAsFrise() throws IOException {
        FileInputStream inputStream = new FileInputStream("frise.txt");
        ...
        while ((line = br.readLine()) != null) {
            ...
            ItemState itemState;
            if (splitted[2].equals("AVAILABLE")) itemState = ItemState.AVAILABLE;
            else itemState = ItemState.UNAVAILABLE;
            item.changeState(itemState);
            listOfFrise.add(item);
        }
        inputStream.close();
    }

    public void viewItems() {
        System.out.println("List of frise:");
        for (Item eachItem : listOfFrise) {
    ...
    public void addItemByAdmin(String itemName, int price) {
        ...
        if (newItem != null) System.out.println("This item is already exist.");
        else {
            newItem = new Item(itemName, price);
            try (PrintWriter output = new PrintWriter(new FileWriter("frise.txt", true))) {
                output.printf("%s\r\n", itemName + " " + price + " " + "AVAILABLE");
                listOfFrise.add(newItem);
    ...
    public void removeItemByAdmin(String itemName) throws IOException {
        ...
        if (deletedItem == null) System.out.println("This item is not exist.");
        else {
            String deletedLine = deletedItem.getItemName() + " " + deletedItem.getPrice() + " " + deletedItem.getState();
            ManageItemInEveryCategory.deleteLine("frise.txt", deletedLine);
            listOfFrise.remove(deletedItem);
    ...
    public void changeStateOfItemByAdmin(String itemName, ItemState state) throws IOException {
        ...
        if (item == null) System.out.println("This item not found.");
        else {
    ...
    public Item getItemByName(String itemName) {
        for (Item eachItem : listOfFrise) {
            if (eachItem.getItemName().equals(itemName)) return eachItem;
        }
        return null;
    }
